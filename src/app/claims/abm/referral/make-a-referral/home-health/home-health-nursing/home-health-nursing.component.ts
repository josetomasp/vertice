...
@Component({
  selector: 'healthe-home-health-nursing',
  templateUrl: './home-health-nursing.component.html',
  styleUrls: ['./home-health-nursing.component.scss'],
  providers: [provideWizardBaseStep(HomeHealthNursingComponent)]
})
export class HomeHealthNursingComponent extends WizardBaseStepDirective
  implements OnInit {
  selectedService = HOMEHEALTH_ARCH_TYPE;

  nursingOptions$ = this.store$.pipe(
    select(getFusionServiceGroupOptionsByGroupName(FusionServiceName.HomeHealth, HOMEHEALTH_ARCH_TYPE), 'Nursing'),
  ...
  specificDateConfig: SpecificDateFormArrayConfig = {
    ...
    addDateButtonLabel: 'Add Nursing Service',
    addLocationButtonLabel: '',
    controls: this.controlConfig.getControls(),
    deleteItemModal: {
      deleteItemMessage:
        'Are you sure you want to delete the selected Nursing Service?',
      deleteItemTitle: 'Delete Nursing Service'
    },
    formTitle: 'Nursing Services',
    hideAddLocationButton: true,
    serviceActionType: 'Nursing',
  ...
  constructor(
    ...
    super(HOMEHEALTH_NURSING_STEP_NAME, store$, wizard, matStepper);
