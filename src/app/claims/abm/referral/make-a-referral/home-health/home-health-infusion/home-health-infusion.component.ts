...
@Component({
  selector: 'healthe-home-health-infusion',
  templateUrl: './home-health-infusion.component.html',
  styleUrls: ['./home-health-infusion.component.scss'],
  providers: [provideWizardBaseStep(HomeHealthInfusionComponent)]
})
export class HomeHealthInfusionComponent extends WizardBaseStepDirective
  implements OnInit {
  selectedService = HOMEHEALTH_ARCH_TYPE;

  infusionOptions$ = this.store$.pipe(
    select(getFusionServiceGroupOptionsByGroupName(FusionServiceName.HomeHealth, HOMEHEALTH_ARCH_TYPE), 'Infusion'),
  ...
  specificDateConfig: SpecificDateFormArrayConfig = {
    ...
    addDateButtonLabel: 'Add Infusion Service',
    addLocationButtonLabel: '',
    controls: this.controlConfig.getControls(),
    deleteItemModal: {
      deleteItemMessage:
        'Are you sure you want to delete the selected Infusion Service?',
      deleteItemTitle: 'Delete Infusion Service'
    },
    formTitle: 'Infusion Services',
    hideAddLocationButton: true,
    serviceActionType: 'Infusion',
  ...
  constructor(
    ...
    super(HOMEHEALTH_INFUSION_STEP_NAME, store$, wizard, matStepper);
