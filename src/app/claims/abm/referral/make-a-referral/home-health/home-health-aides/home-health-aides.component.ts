...
@Component({
  selector: 'healthe-home-health-aides',
  templateUrl: './home-health-aides.component.html',
  styleUrls: ['./home-health-aides.component.scss'],
  providers: [provideWizardBaseStep(HomeHealthAidesComponent)]
})
export class HomeHealthAidesComponent extends WizardBaseStepDirective
  implements OnInit {
  selectedService = HOMEHEALTH_ARCH_TYPE;

  aidesOptions$ = this.store$.pipe(
    select(getFusionServiceGroupOptionsByGroupName(FusionServiceName.HomeHealth, HOMEHEALTH_ARCH_TYPE), 'Aides'),
  ...
  specificDateConfig: SpecificDateFormArrayConfig = {
    ...
    addDateButtonLabel: 'Add Aide Service',
    addLocationButtonLabel: '',
    controls: this.controlConfig.getControls(),
    deleteItemModal: {
      deleteItemMessage:
        'Are you sure you want to delete the selected Aide Service?',
      deleteItemTitle: 'Delete Aide Service'
    },
    formTitle: 'Aide Services',
    hideAddLocationButton: true,
    serviceActionType: 'Aide',
  ...
  constructor(
    ...
    super(HOMEHEALTH_AIDES_STEP_NAME, store$, wizard, matStepper);
