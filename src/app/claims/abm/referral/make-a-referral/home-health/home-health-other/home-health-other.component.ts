...
@Component({
  selector: 'healthe-home-health-other',
  templateUrl: './home-health-other.component.html',
  styleUrls: ['./home-health-other.component.scss'],
  providers: [provideWizardBaseStep(HomeHealthOtherComponent)]
})
export class HomeHealthOtherComponent extends WizardBaseStepDirective
  implements OnInit {
  selectedService = HOMEHEALTH_ARCH_TYPE;

  otherOptions$ = this.store$.pipe(
    select(getFusionServiceGroupOptionsByGroupName(FusionServiceName.HomeHealth, HOMEHEALTH_ARCH_TYPE), 'Other'),
  ...
  specificDateConfig: SpecificDateFormArrayConfig = {
    ...
    deleteItemModal: {
      deleteItemMessage:
        'Are you sure you want to delete the selected Other Service?',
      deleteItemTitle: 'Delete Other Service'
    },
    formTitle: 'Other Home Health Services',
  ...
  constructor(
    ...
    super(HOMEHEALTH_OTHER_STEP_NAME, store$, wizard, matStepper);
