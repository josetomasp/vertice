...
@Component({
  selector: 'healthe-diagnostics-other',
  templateUrl: './diagnostics-other.component.html',
  styleUrls: ['./diagnostics-other.component.scss'],
  providers: [provideWizardBaseStep(DiagnosticsOtherComponent)]
})
export class DiagnosticsOtherComponent extends WizardDateStepComponent
  implements OnInit {
  @Input()
  locations$;
  otherTypes$ = this.store$.pipe(
    select(
      getFusionServiceGroupOptionsByGroupName(
  ...
  specificDateConfig: SpecificDateFormArrayConfig = {
    formTitle: 'Other Diagnostic Services',
    serviceActionType: 'Other',
    stepName: this.stepName,
    addDateButtonLabel: 'Add Other Diagnostic Service',
    ...
    deleteItemModal: {
      deleteItemTitle: 'Delete Other Diagnostic Service?',
      deleteItemMessage:
        'Are you sure that you would like to remove the selected Other Diagnostic Service?'
    },
    controls: [
      [
        {
          ...
          formControlName: 'otherTypes',
          validatorOrOpts: Validators.required,
          errorMessages: { required: 'Select a Service' },
          formState: null,
          options: this.otherTypes$,
    ...
    this.stepForm$
      ...
      .subscribe((formValues: DiagnosticsFormState) => {
        formValues.schedulingForm.map((schedulingForm) => {
          schedulingForm.otherTypes = schedulingForm.subType;
        });
